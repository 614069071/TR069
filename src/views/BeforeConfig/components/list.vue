<template>
  <div class="layout-page-view-wrapper">
    <div class="layout-page-view-controls">
      <a-button type="primary" @click="jumpTo('add')">新增工单</a-button>
      ...
      <template #columns>
        ...
        <a-table-column title="操作" width="150">
          <template #cell="{ record }">
            <control-buttons>
              <span class="primary-color" @click="jumpTo('detail', record)">详情</span>
...
export default defineComponent({
  ...
  setup() {
    ...
    const jumpTo = (action, data) => {
      const link = `/layout/deviceManagement/beforeConfig/${action}`;
      router.push(link);
      data && sessionStorage.setItem("_pre_config_item_", JSON.stringify(data));
    };
