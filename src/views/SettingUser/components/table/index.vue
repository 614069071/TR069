<template>
  <div class="asasdads">
    <div>
      <a-table :data="data" :scroll="{ y: 650 }" :pagination="false">
        <template #columns>
          <a-table-column title="序号" data-index="index"></a-table-column>
          <a-table-column title="用户名" data-index="username"></a-table-column>
          <a-table-column title="角色" data-index="roleNameZh"></a-table-column>
          <a-table-column title="描述" data-index="description"></a-table-column>
          <a-table-column title="状态">
            <template #cell="{ record }">
              <div class="cell" v-if="record.enable === 3" style="color: #67c23a">在线</div>
              <div class="cell" v-if="record.enable === 4" style="color: #f56c6c">离线</div>
              <div class="cell" v-if="record.enable === 2" style="color: #e6a23c">待确认</div>
              <div class="cell" v-if="record.enable === 1" style="color: #67c23a">启用</div>
              <div class="cell" v-if="record.enable === 0" style="color: #f56c6c">禁用</div>
            </template>
          </a-table-column>
          <a-table-column title="启用">
            <template #cell="{ record }">
              <a-switch v-model="record.enableType" checked-value="1" unchecked-value="0" @change="changeEnabled(record)" />
            </template>
          </a-table-column>
          <a-table-column title="操作" width="300">
            <template #cell="{ record }">
              <span @click="handleClick(record)" v-show="record.parentId != 0" class="dalst bod">修改</span>
              <span @click="handleDetail(record)" class="dalst bod">详情</span>
              <span @click="changePassword(record)" v-show="record.parentId != 0" class="dalst bod">修改密码</span>
              <span class="dalst col" v-show="record.parentId != 0" @click="handleDelete(record)">删除</span>
            </template>
          </a-table-column>
        </template>
      </a-table>
      ...
      <a-modal v-model:visible="visible" @cancel="handleCancel" @before-ok="handleBeforeOk">
        ...
        <div class="changePasswordCss">
          ...
          <a-input v-model="Password" type="password" placeholder="please enter..." />
        </div>
        <div class="changePasswordCss">
          <span class="label">确认密码</span>
          <a-input v-model="confirmPassword" type="password" placeholder="please enter..." />
        </div>
      </a-modal>
    </div>
  </div>
